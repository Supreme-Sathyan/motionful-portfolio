...
const INTRO_MESSAGE: Message = {
  id: 'intro',
  role: 'assistant',
  content: "Hi, I'm Sathyan's portfolio assistant. Ask me about projects, research, or achievements.",
  timestamp: new Date(),
};
...
const panelVariants = {
  ...
  exit: { opacity: 0, y: 24, scale: 0.98, transition: { duration: 0.35, ease: EASE_SMOOTH } },
};
...
export default function Chatbot() {
  ...
  const sendMessage = async (text: string) => {
    ...
    const historyForAPI = [...messages.filter((m) => m.id !== 'intro'), userMsg].map((m) => ({
      role: m.role,
      content: m.content,
    }));
    ...
    await streamChat({
      ...
      onError: (error) => {
        setMessages((prev) => [
          ...prev,
          { id: crypto.randomUUID(), role: 'assistant', content: error, timestamp: new Date() },
        ]);
        setIsLoading(false);
      },
    });
      ...
      <AnimatePresence>
        {!isOpen && (
          ...
          >
            ...
            <span className="text-sm font-medium tracking-tight">Ask about me</span>
          </motion.button>
        )}
      </AnimatePresence>
                ...
                >
                  <div className="bg-secondary/60 rounded-xl rounded-bl-sm px-4 py-3">
                    <div className="flex items-center gap-1.5">
                      <span className="text-xs text-muted-foreground font-mono tracking-wide">Thinking</span>
